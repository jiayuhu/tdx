...
前置条件:
1. 通达信客户端已启动并登录
2. 已在客户端中创建 X01_BELOW240W 选股公式

使用示例:
    python xg_below240w.py
"""
...
# 从配置文件读取
# ============================================================================
xg_config = config.get("xg_below240w", {})
SOURCE_BLOCK = xg_config.get("source_block", "AAA")
TARGET_BLOCK = xg_config.get("target_block", "P001")
TARGET_BLOCK_NAME = xg_config.get("target_block_name", "低于 240 周线")
...
def select_by_formula(stock_list: list) -> list:
    """
    使用通达信公式引擎批量选股
    """
    selected = []
    ...
    print()
    print("=" * 70)
    print("选股公式：X01_BELOW240W（公式引擎版）")
    print("条件：收盘价低于 240 周均线")
    print("复权：前复权")
    print("过滤：ST 股票")
    ...
    print(f"原始股票：{len(stock_list)} 只，过滤 ST 股票：{st_count} 只，剩余：{len(non_st_list)} 只")
    print()
    ...
    for batch_idx, batch in enumerate(batches, 1):
        batch_end = min(batch_idx * BATCH_SIZE, len(non_st_list))
        ...
        print(f"进度：{batch_end}/{len(non_st_list)} ({batch_end*100//len(non_st_list)}%) - 批次 {batch_idx}/{len(batches)} - 选中 {batch_selected_count} 只")
    
    print()
    print("=" * 70)
    print("选股结果")
    print("=" * 70)
    print(f"源板块：共 {len(stock_list)} 只股票")
    print(f"选中股票：{len(selected)} 只 ({len(selected)*100//len(stock_list) if stock_list else 0}%)")
    print()
...
def main():
    ...
    print()
    print("=" * 70)
    print(f"选股任务：从板块 {source_block} 筛选收盘价低于 240 周均线的股票")
    print("=" * 70)
    print()
    ...
    # 3. 执行选股
    selected_stocks = select_by_formula(stock_list)
    ...
    print("=" * 70)
    if selected_stocks:
        print(f"选中 {len(selected_stocks)} 只股票已添加到板块 '{target_block}'")
